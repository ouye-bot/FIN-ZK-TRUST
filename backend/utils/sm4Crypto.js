...
async function encryptFields(tableName, data, userId) {
  if (tableName === 'users') {
    ...
    if (data.amount !== undefined && data.amount !== null) {
      ...
      data.amount = await encrypt(String(Number(data.amount)), userId, aad);
    }
    if (data.interest !== undefined && data.interest !== null) {
      const aad = buildAAD('transactions', 'interest', userId);
      data.interest = await encrypt(String(Number(data.interest)), userId, aad);
    }
    if (data.total_amount !== undefined && data.total_amount !== null) {
      const aad = buildAAD('transactions', 'total_amount', userId);
      data.total_amount = await encrypt(String(Number(data.total_amount)), userId, aad);
    }
  }
  return data;
}
...
async function decryptFields(tableName, data, userId) {
  ...
  if (tableName === 'users') {
    ...
    if (data.credit_score !== undefined && data.credit_score !== null) {
      try {
        const aad = buildAAD('users', 'credit_score', userId);
        const decrypted = await decrypt(data.credit_score, userId, aad);
    ...
    if (data.amount !== undefined && data.amount !== null) {
      try {
        const aad = buildAAD('transactions', 'amount', userId);
        const decrypted = await decrypt(data.amount, userId, aad);
    ...
    if (data.interest !== undefined && data.interest !== null) {
      try {
        const aad = buildAAD('transactions', 'interest', userId);
        const decrypted = await decrypt(data.interest, userId, aad);
    ...
    if (data.total_amount !== undefined && data.total_amount !== null) {
      try {
        const aad = buildAAD('transactions', 'total_amount', userId);
        const decrypted = await decrypt(data.total_amount, userId, aad);
